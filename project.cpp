...
int NO_PARENT = -1;
int i = 0;
void printPath(int currentVertex, vector<int>& parents, int start_vertex, vector<vector<vector<int>>>& all_info) {
    if (currentVertex == NO_PARENT) {
        i = 0;
        return;
    }
    printPath(parents[currentVertex], parents, start_vertex, all_info);
    all_info[start_vertex][currentVertex][i++] = currentVertex;
    cout << currentVertex << " ";
    
}

void printSolution(int startVertex, vector<int>& distances, vector<int>& parents, vector<vector<vector<int>>>& all_info) {
    ...
    for (int vertexIndex = 0; vertexIndex < nVertices; vertexIndex++) {
        if (vertexIndex != startVertex) {
            ...
            printPath(vertexIndex, parents, startVertex, all_info);
        }
    }
}

void dijkstra(vector<vector<int>>& adjacencyMatrix, int startVertex, vector<vector<vector<int>>>& all_info) {
    ...
    printSolution(startVertex, shortestDistances, parents, all_info);
}
...
int main() {
    ...
    for (int i = 0; i < b; i++) {
        dijkstra(adjacencyMatrix, route[i], all_info);
        cout << endl;
    }
